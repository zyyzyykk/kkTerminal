...
> 命令代码是用于访问和执行特定操作流程的快捷方式
>
> 命令代码长度为 2~6 位，其中第一位表示命令代码类型
>
> 命令代码不区分大小写

##### 命令代码类型

在命令代码输入框中输入 `SCCC` 并按下回车以查看更多信息

- 功能命令代码：以 `F` 开头，用于执行通用功能
- 系统命令代码：以 `S` 开头，用于访问系统模块
- 用户命令代码：以 `U` 开头，用于自定义工作流

##### 新建用户命令代码

1. 在命令代码输入框中输入 `SCCW` 并按下回车
2. 输入命令代码名称与描述，并编写相应的工作流
3. 点击确定添加用户命令代码
...
> 导入：导入任意数量的用户命令代码（会覆盖同名的命令代码）
>
> 导出：导出当前已存在的全部用户命令代码
>
> 注意：导入导出用户命令代码的文件格式均为 json
